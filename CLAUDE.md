# QuickNote — Obsidian 연동 포스트잇 메모 앱
...
| 이름 | 메모지 bg | 타이틀바 bar |
|---|---|---|
| Sunnyside | #FFFBEA | #FFF3A3 |
| Limeade | #F2FAE8 | #CDEEA0 |
| Blue Paradise | #E8F6FB | #A4DCEE |
| Positively Pink | #FFF0F5 | #FFD0E2 |
| Guava | #FFF0EC | #FFBDAC |

#### 4. 저장 방식
- **X 버튼** 클릭 시 자동 저장 후 닫힘 (내용 없으면 그냥 닫힘)
- **Ctrl+S**: 수동 저장 (창 유지)
- 저장 경로: `{VaultPath}/YYYY-MM-DD-HHmm.md` (Vault 루트에 바로 저장)
- Frontmatter 자동 생성: `created`, `tags` (본문의 `#태그` 파싱)

#### 5. 타이틀바 메뉴 구성 (SVG 아이콘)
왼쪽: 색상 스와치 5개 (사각형)
오른쪽: +새메모 / 핀(항상위) / 최소화 / 닫기
...
빌드 결과물: `dist/QuickNote-win32-x64/QuickNote.exe`
...
| 채널명 | 방향 | 설명 |
|---|---|---|
| `note:save` | renderer→main | 메모 저장 요청 |
| `config:get` | renderer→main | 설정 조회 |
| `config:set` | renderer→main | 설정 저장 (autoStart 포함) |
...
| `window:bounds` | renderer→main | 현재 창 좌표/크기 조회 |
...
- 타이틀바 hover 감지: `document.mousemove` + `e.clientY <= 40` (hover-zone div 방식은 z-index가 버튼 클릭을 막음)
- 색상 변경 시 `applyColor()` 함수가 body.background + titlebar.background + swatch.active 동기 처리
- 새 창 생성 시 `nextColorIndex` 순환하여 색상 자동 할당
- 저장 경로는 QuickNotes 서브폴더 없이 Vault 루트에 직접 저장
